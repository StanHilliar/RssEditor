  ...
  ***/
  ...
  $scope.onAddModuleToEmail = function (moduleId, dropzoneModuleIndex, position) 
  {
    ...
    if(moduleIndex != -1)
    {
      ...
      $scope.entity.modules[dropzoneModuleIndex].modules[position] = $scope.entity.dropzoneModules[moduleIndex];

      initModule($scope.entity.modules[dropzoneModuleIndex].modules[position], dropzoneModuleIndex, position).then(function() 
      {
  ...
  $scope.generateModule = function(isEdit, currentModule, moduleData, moduleFeedPositions, moduleCounter, moduleDataStr, moduleIdStr)
  {
    ...
    {
      moduleHtml += '<div id="modulesContainer" data-dropzone-id='+moduleIdStr+'>';
      moduleHtml += $scope.generateContainer(isEdit, currentModule, moduleData, moduleDataStr, moduleIdStr+'_C_');
      moduleHtml += '</div>';
    }
    else
    {
      ...
      //for(var  x = 0; (x < moduleFeedPositions.length && x < $scope.feedNumberOfEntries.no); x++)
      ...
      for (var x = 0; (x < moduleFeedPositions.length && x < moduleData.numberOfEntries); x++) 
      {
        ...
        if (entryState == 1 || isEdit) 
        {
          ...
          //var _entryView = currentModule.views[moduleData[i]._view.id].source;
          ...
          for (var varCounter = 0; varCounter < currentModule.variables.length; varCounter++) 
          {
            ...
            // _entryView = _entryView.replace("/\{\{"+variableName+"\}\}/g", '{{moduleDataStr.data['+i+'].data.'+variableName+'}}');
          }
  ...
  $scope.generateContainer = function(isEdit, container, data, dataStr, containerIdStr)
  {
    console.log('generateContainer')
    var formatedEntries = '';
    if(container && container.modules)
    {
      for (var moduleCounter = 0; moduleCounter < container.modules.length; moduleCounter++) 
      {
        //console.log('module '+moduleCounter);
        var noSkippedEntries = 0;
        var moduleState = data[moduleCounter].state;
        //console.log( $scope.entity.modules[moduleCounter].childTagName);

        if (moduleState == 1 || isEdit) 
        {
          var _moduleData = $scope.generateModule(isEdit, container.modules[moduleCounter], data[moduleCounter], $scope.feedPositions[moduleCounter], moduleCounter, dataStr+'['+moduleCounter+']', containerIdStr+moduleCounter);
    ...
    //   if (moduleState == 1 || isEdit) 
    //   {
    //     var _moduleData = $scope.generateModule(isEdit, container.modules[moduleCounter], moduleCounter);
  ...
  $scope.generateEmail = function (isEdit) 
  {
    ...
    formatedEntries += $scope.generateContainer(isEdit, $scope.entity, $scope.rssData, 'rssData', '');
  ...
  $scope.loadManualModule = function (currentModule, modulePos, subModulePos, from, to) 
  {
    ...
    //   $scope.rssData[modulePos].data[i]._view     = currentModule.views[0];
    ...
    //   {
    //     $scope.rssData[modulePos].data[i].data[currentModule.variables[x].name]      = currentModule.variables[x].defaultValue;
    //     $scope.rssData[modulePos].data[i].variables[currentModule.variables[x].name] = currentModule.variables[x];
    //   }
