...
/* jshint -W098 */
angular.module('mean.emaileditor').controller('EmaileditorController', ['$scope', '$q', '$stateParams', '$compile', '$interpolate', '$sce', 'Global', 'Emaileditor', 'NewsletterEntity', 'Email', 'EloquaService', 'XMLFeed', 'MeanUser', '$filter',
  function ($scope, $q, $stateParams, $compile, $interpolate, $sce, Global, Emaileditor, NewsletterEntity, Email, EloquaService, XMLFeed, MeanUser, $filter) 
  {
    $scope.global = Global;
    $scope.package =
    {
    ...
    var EloquaEmail               = EloquaService.eloquaEmail();
    var EloquaCampaign            = EloquaService.eloquaCampaign();
    var EloquaTestEmail           = EloquaService.eloquaTestEmail();
    var EloquaCampaignUnschedule  = EloquaService.eloquaCampaignUnschedule();
    ...
    {
      Emaileditor.getEmailTemplate().query({ company: $stateParams.company, entityId: $stateParams.newsletterid }, function (newsletterEntityArray) 
      {
        ...
        if($scope.segment == '') 
        {
          ...
          if ($stateParams.emailid != null) 
          {
            ...
            Email.query({ company: $stateParams.company, emailId: $stateParams.emailid }, function (emails) 
            {
              var email = emails[0];
    ...
    ***/
    ...
    function saveEloquaEmail(cb, firstTry) 
    {
      ...
      $scope.checkAds(function () 
      {
        ...
        storedEloquaEmail.company = $stateParams.company;
    ...
    $scope.scheduleEmail = function () 
    {
      ...
      $scope.saveEmail(function () 
      {
        ...
        storedEloquaCampaign.company = $stateParams.company;
    ...
    $scope.updateModulePositions = function(dropzoneId, oldIndex, newIndex)
    {
      // console.log('-- updateModulePositions --');
      // console.log(dropzoneId);
      $scope.modulePositions[dropzoneId]  = moveInArray($scope.modulePositions[dropzoneId],oldIndex, newIndex);
    };
    ...
    $scope.clickOnEmailModule = function (elementID) 
    {
      ...
      if(containerId)
      {
        ...
        $scope.selectedModule.data   = $scope.rssData[containerId][_moduleID];
        $scope.selectedModule.module = $scope.entity.modules[containerId].modules[_moduleID];
      }
      else
      {
        $scope.selectedModule.data = $scope.rssData[_moduleID];
        $scope.selectedModule.module = $scope.entity.modules[_moduleID];
      }

      $scope.$apply();
    };

    $scope.addEntry = function (containerId, moduleId) 
    {
      console.log("addEntry("+containerId+','+ moduleId+")");
      var _module = $scope.selectedModule.module;
      var _data   = $scope.selectedModule.data;
      ...
      if (_module.type == "3")
      {
        $scope.loadManualModule(_module, _containerId, _moduleId, (parseInt(_data.numberOfEntries)), parseInt(_data.numberOfEntries) + 1).then(function(moduleData)
        {
          addEntryCallback(moduleData, _containerId, _moduleId);
        });
      }

      if (_module.type == "2")
      {
        $scope.loadXMLModule(_module, _containerId, _moduleId, (parseInt(_data.numberOfEntries)), parseInt(_data.numberOfEntries) + 1).then(function(moduleData)
        {
          addEntryCallback(moduleData, _containerId, _moduleId);
        });
      }
      ...
      if (_module.type == "1")
      {          
        $scope.loadRSSModule(_module, _containerId, _moduleId, (parseInt(_data.numberOfEntries)), parseInt(_data.numberOfEntries) + 1).then(function(moduleData)
        {
          addEntryCallback(moduleData, _containerId, _moduleId);
        });
      }
    ...
    function addEntryCallback(moduleData, containerId, moduleId)
    {
      console.log('addEntryCallback(%s,%s)',containerId, moduleId);

      console.log(moduleData);
      if(moduleData.subModulePos)
      {
        for(var i = 0; i < moduleData.entries.length; i++) 
        {
          if(moduleData.entries[i])
          {
            $scope.rssData[moduleData.modulePos][moduleData.subModulePos].data.push(moduleData.entries[i]);
          }
        }

        for(var i = 0; i < moduleData.positions.length; i++) 
        {
          if(moduleData.positions[i])
          {
            $scope.feedPositions[moduleData.modulePos][moduleData.subModulePos].push(moduleData.positions[i]);
          }
        }
      }
      else
      {
        for(var i = 0; i < moduleData.entries.length; i++) 
        {
          if(moduleData.entries[i])
          {
            $scope.rssData[moduleData.modulePos].data.push(moduleData.entries[i]);
          }
        }
        ...
        for(var i = 0; i < moduleData.positions.length; i++) 
        {
          if(moduleData.positions[i])
          {
            $scope.feedPositions[moduleData.modulePos].push(moduleData.positions[i]);
          }
        }
      }
      
      if(containerId != null && containerId != undefined && moduleId != null && moduleId != undefined)
      {
        $scope.rssData[containerId][moduleId].numberOfEntries++;
      }
      else
      {
        $scope.rssData[containerId].numberOfEntries++;
      }
    ...
    $scope.generateModule = function (isEdit, currentModule, moduleData, moduleAdData, moduleFeedPositions, moduleCounter, moduleDataStr, adDataStr, moduleIdStr)
    {
      ...
      if (currentModule)
      {
        ...
        if (isEdit) 
        {
          moduleHtml += '<div data-ng-hide="' + moduleDataStr + '.state==0" id="emailModule_' + moduleIdStr + '" data-module-id="'+currentModule._id+'" data-ng-class="{emailModule:true, inactiveEmailModule:' + moduleDataStr + '.state==2}">';
          moduleHtml += '<div class="emailModuleHandle" id="emailModuleHandle_' + moduleIdStr + '"><i class="material-icons">open_with</i></div>';
          moduleHtml += '<div class="emailModuleSelector" id="emailModuleSelector_' + moduleIdStr + '"><i class="material-icons">settings</i></div>';
        }
        ...
        //if the preBody is empty add a spacer to be able to drop modules before it
        if(isEdit && ((preBody && preBody.trim() == '')  || !preBody) )
        {
          console.log('ADD spaCer');
          preBody = '<div class="moduleSpacer"></div>';
        }
        ...
        if(currentModule.bodyVariables)
        {
         ...
         //if the postBody is empty add a spacer to be able to drop modules before it
         if(isEdit && ((postBody && postBody.trim() == '')  || !postBody) )
         {
           console.log('ADD spaCer');
           postBody = '<div class="moduleSpacer"></div>';
         }
    ...
    $scope.generateContainer = function (isEdit, container, data, adData, modulePositions, containerFeedPositions, dataStr, adDataStr, containerIdStr)
    {
      ...
      {
        // console.log('moduleCounter '+moduleCounter);
        // console.log('moduleId '+modulePositions[moduleCounter].moduleId);
    ...
    {
      console.log('loadRSSModule(%s,%s,%s,%s)', modulePos, subModulePos, from, to);
      var deferred = $q.defer();
      ...
      console.log('rssFeed.length:'+rssFeed.length);
